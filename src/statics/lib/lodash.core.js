...
(function () {
  ...
  function baseValues(object, props) {
    return baseMap(props, key => object[key]);
  }
  ...
  let nativeIsFinite = root.isFinite,
  ...
  function baseFunctions(object, props) {
    return baseFilter(props, key => isFunction(object[key]));
  }
  ...
  function baseIsEqualDeep(
    ...
    stack || (stack = []);
    const objStack = find(stack, entry => entry[0] == object);
    const othStack = find(stack, entry => entry[0] == other);
  ...
  function baseWrapperValue(value, actions) {
    ...
    return reduce(
      actions,
      (result, action) => action.func.apply(
        action.thisArg,
  ...
  function equalArrays(array, other, bitmask, customizer, equalFunc, stack) {
    ...
    while (++index < arrLength) {
      ...
      if (seen) {
        if (
          !baseSome(other, (othValue, othIndex) => {
            if (
  ...
  function sortBy(collection, iteratee) {
    ...
    return baseMap(
      baseMap(collection, (value, key, collection) => ({
        value,
        index: index++,
   ...
   * // => 'hi fred!'
   */
  const bind = baseRest((func, thisArg, partials) => createPartial(
    func,
   ...
   */
  const defer = baseRest((func, args) => baseDelay(func, 1, args));
   ...
   */
  const delay = baseRest((func, wait, args) => baseDelay(func, toNumber(wait) || 0, args));
  ...
  function mixin(object, source, options) {
    ...
    baseEach(methodNames, (methodName) => {
  ...
  baseEach(
    ...
    (methodName) => {
      let func = (/^(?:replace|split)$/.test(methodName)
          ? String.prototype
