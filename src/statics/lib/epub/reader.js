...
(function (root, $) {
  const previousReader = root.ePubReader || {};

  const ePubReader = root.ePubReader = function (path, options) {
...
EPUBJS.Reader.prototype.selectedRange = function (range) {
  ...
  const cfi = epubcfi.generateCfiFromRangeAnchor(range, this.book.renderer.currentChapter.cfiBase);
...
EPUBJS.reader.BookmarksController = function () {
  ...
  const createBookmarkItem = function (cfi) {
  ...
  _.each(this.settings.bookmarks, (cfi) => {
    const bookmark = createBookmarkItem(cfi);
    docfrag.appendChild(bookmark);
  });

  $list.append(docfrag);

  this.on('reader:bookmarked', (cfi) => {
    const item = createBookmarkItem(cfi);
    $list.append(item);
  });

  this.on('reader:unbookmarked', (index) => {
...
EPUBJS.reader.ControlsController = function (book) {
  ...
  $bookmark.on('click', () => {
    const cfi = reader.book.getCurrentLocationCfi();
...
EPUBJS.reader.NotesController = function () {
  ...
  const annotations = reader.settings.annotations;
  const renderer = book.renderer;
  const popups = [];
  const epubcfi = new EPUBJS.EpubCFI();
  ...
  _.each(annotations, (note) => {
    addAnnotation(note);
  });


  renderer.registerHook('beforeChapterDisplay', (callback, renderer) => {
    const chapter = renderer.currentChapter;
    _.each(annotations, (note) => {
      const cfi = epubcfi.parse(note.anchor);
...
EPUBJS.reader.ReaderController = function (book) {
  ...
  const slideIn = function () {
    const currentPosition = book.getCurrentLocationCfi();
...
EPUBJS.reader.SettingsController = function () {
  ...
  const $sidebarReflowSetting = $('#sidebarReflow');

  $sidebarReflowSetting.on('click', () => {
    reader.settings.sidebarReflow = !reader.settings.sidebarReflow;
  });

  $settings.find('.closer').on('click', () => {
    hide();
  });
...
EPUBJS.reader.SidebarController = function (book) {
  ...
  const changePanelTo = function (viewName) {
    const controllerName = `${viewName}Controller`;
...
EPUBJS.reader.TocController = function (toc) {
  ...
  const chapterChange = function (e) {
    let id = e.id,
  ...
  const tocitems = generateTocItems(toc);
