EPUBJS.Hooks.register('beforeChapterDisplay').endnotes = function (a, b) {
  let c = b.contents.querySelectorAll('a[href]'),
    d = Array.prototype.slice.call(c),
    e = EPUBJS.core.folder(location.pathname),
    f = (EPUBJS.cssPath, {}); EPUBJS.core.addCss(`${EPUBJS.cssPath}popup.css`, !1, b.render.document.head), d.forEach((a) => {
    function c() {
      let c,
        h,
        n = b.height,
    ...
    } function d() { f[i].classList.add('on'); } function e() { f[i].classList.remove('on'); } function g() { setTimeout(() => { f[i].classList.remove('show'); }, 100); } let h,
      i,
      j,
      k,
      l,
      m; a.getAttribute('epub:type') == 'noteref' && (h = a.getAttribute('href'), i = h.replace('#', ''), j = b.render.document.getElementById(i), a.addEventListener('mouseover', c, !1), a.addEventListener('mouseout', g, !1));
  }), a && a();
}, EPUBJS.Hooks.register('beforeChapterDisplay').mathml = function (a, b) { if (b.currentChapter.manifestProperties.indexOf('mathml') !== -1) { b.render.iframe.contentWindow.mathmlCallback = a; const c = document.createElement('script'); c.type = 'text/x-mathjax-config', c.innerHTML = '        MathJax.Hub.Register.StartupHook("End",function () {           window.mathmlCallback();         });        MathJax.Hub.Config({jax: ["input/TeX","input/MathML","output/SVG"],extensions: ["tex2jax.js","mml2jax.js","MathEvents.js"],TeX: {extensions: ["noErrors.js","noUndefined.js","autoload-all.js"]},MathMenu: {showRenderer: false},menuSettings: {zoom: "Click"},messageStyle: "none"});                 ', b.doc.body.appendChild(c), EPUBJS.core.addScript('http://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS-MML_HTMLorMML', null, b.doc.head); } else a && a(); }, EPUBJS.Hooks.register('beforeChapterDisplay').smartimages = function (a, b) {
  let c = b.contents.querySelectorAll('img'),
    d = Array.prototype.slice.call(c),
    e = b.height; if (b.layoutSettings.layout != 'reflowable') return void a(); d.forEach((a) => {
    let c = function () {
        let c,
      ...
      d = function () { b.off('renderer:resized', c), b.off('renderer:chapterUnload', this); }; a.addEventListener('load', c, !1), b.on('renderer:resized', c), b.on('renderer:chapterUnload', d), c();
  }), a && a();
}, EPUBJS.Hooks.register('beforeChapterDisplay').transculsions = function (a, b) {
  const c = b.contents.querySelectorAll('[transclusion]'); Array.prototype.slice.call(c).forEach((a) => {
    function c() { j = g, k = h, j > chapter.colWidth && (d = chapter.colWidth / j, j = chapter.colWidth, k *= d), f.width = j, f.height = k; } var d,
